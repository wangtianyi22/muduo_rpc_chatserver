...
ChatService::ChatService()
{
    ...
    _msgHandlerMap.insert({CREATE_GROUP_MSG, std::bind(&ChatService::createGroup, this, 
                        std::placeholders::_1, std::placeholders::_2, std::placeholders::_3)});
    _msgHandlerMap.insert({ADD_FRIEND_MSG, std::bind(&ChatService::addGroup, this, 
                        std::placeholders::_1, std::placeholders::_2, std::placeholders::_3)});
...
MsgHandler ChatService::getHandler(int msgid)
{
    ...
    if (it == _msgHandlerMap.end())
    {
        ...
        return [=](const TcpConnectionPtr &conn, json &js, Timestamp) {
            LOG_ERROR(" %s:%s:%d, msgid %d cannot find handler", __FILE__, __FUNCTION__, __LINE__ , msgid);
        };
    }
    else
    {
...
void ChatService::reg(const TcpConnectionPtr &conn, json &js, Timestamp time){
    LOG_INFO("%s:%s:%d, do register", __FILE__, __FUNCTION__, __LINE__);
...
// 处理客户端异常退出   2个操作  1.从在线用户表中删除 2.更新用户的offline状态
...
{   
...

    {
        ...
        {
            // toid在线，转发消息   服务器主动推送消息给toid用户,服务器相当于中转站
