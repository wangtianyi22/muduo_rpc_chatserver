...
void RpcProvider::NotifyService(google::protobuf::Service *service)
{
    ...
    // LOG_INFO("service_name:%s", service_name.c_str());   日志没有输出，需要修改一下
...
// 新的socket连接回调，rpc服务是短连接，检测一下连接状态就可以关闭了
                            ...
                            Timestamp)
{
    ...
    if (!request->ParseFromString(args_str))   // 设置请求的对应参数
    {
    ...
    // 在框架上根据远端rpc请求，调用当前rpc节点上发布的方法， 通过抽象的方法执行远端请求得方法得调用
