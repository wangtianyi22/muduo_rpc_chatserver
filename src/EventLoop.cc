...
int createEventfd()
{
    ...
    LOG_INFO("file %s func %s eventfd:%d", __FILE__, __FUNCTION__, evtfd);
    if (evtfd < 0)
    {
...
void EventLoop::quit()
{
    ...
    if (!isInLoopThread())    // 不在自己的线程需要先唤醒一下，才能退出loop
    {
        wakeup();
    }
}
...
void EventLoop::doPendingFunctors() // 执行回调
{
    ...
    {
        std::unique_lock<std::mutex> lock(mutex_);
        functors.swap(pendingFunctors_);  // 执行交换，防止主Reactor添加cb时堵塞
    }
